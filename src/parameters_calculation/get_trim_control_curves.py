from src.parameters_calculation.get_coefs_mom import *
from src.helpers.path import path

Veq_tilde_ = np.linspace(0.00001, 100, 1000)    # To plot a continuous function

""" Import thrust data generated by thrust.exe """
thrusts_m3 =  np.loadtxt(path + '/src/parameters_calculation/matlab_measurements/measurement_3_thrust.dat')
thrusts_m3s = np.loadtxt(path + '/src/parameters_calculation/matlab_measurements/measurement_3s_thrust.dat')
...
def plot_trim_control_curves(filename, show=True):
    ...
    plt.savefig(filename + "_deVe", dpi=250)
    if show:
        plt.show()
    ...
    plt.axhline(y=0, color='k')
    plt.axvline(x=0, color='k')
    plt.savefig(filename + "_FeVe", dpi=250)
    if show:
        plt.show()
